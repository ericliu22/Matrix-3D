...
@click.option('--pretrained', 'pretrained_model_name_or_path', type=str, default='/mnt/workspace/zhongqi.yang/VideoInpainting_new/ViewCrafter/extern/MoGe/checkpoints/model.pt', help='Pretrained model name or path. Default is "Ruicheng/moge-vitl"')
...
def main(
    ...
    for image_path in (pbar := tqdm(image_paths, desc='Inference', disable=len(image_paths) <= 1)):
        ...
        mm = depth<1e6
        print(f"vertices range check: {vertices[...,2].max()} {vertices[...,2].min()} {depth[mm].max()} {depth[mm].min()} {vertices.shape} {depth.shape}")
        print(f"intrinsics check: {intrinsics}")
        if save_glb_:
