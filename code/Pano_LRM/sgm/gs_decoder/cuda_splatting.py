...
def depth_tensor_to_rgb_img(depth_tensor, cmap='viridis'):
    """
    将单个深度图 Tensor (H, W) 转为 RGB 图像 (PIL.Image)。
    """
    ...
    color_map = cmap_func(depth_norm)[..., :3]  # shape: (H, W, 3), ignore alpha
    rgb = (color_map * 255).astype(np.uint8)
    return Image.fromarray(rgb)

def stitch_images(image_list, ncols=None):
    """
    将图像列表拼成一张大图。
    """
    if ncols is None:
        ncols = int(np.ceil(np.sqrt(len(image_list))))
    nrows = int(np.ceil(len(image_list) / ncols))

    w, h = image_list[0].size
    canvas = Image.new('RGB', (w * ncols, h * nrows))
...
def render_cuda(
    ...
    for i in range(b):
        ...
        settings = GaussianRasterizationSettings(
            ...
            bg=background_color[i].to(torch.float),
            scale_modifier=1.0,
        ...
        #original rasterization
        # image, radii, inv_depth = rasterizer(
        #     means3D=gaussian_means[i],
        #     means2D=mean_gradients,
        #     shs=shs[i] if use_sh else None,
        #     colors_precomp=None if use_sh else shs[i, :, 0, :],
        ...
        #     cov3D_precomp=gaussian_covariances[i, :, row, col],
        #     )
        # depth = torch.zeros_like(image[..., 0], dtype=torch.float32)
        all_images.append(image)
        all_radii.append(radii)

        all_depths.append(depth.squeeze(0))
    # rgb_images = [depth_tensor_to_rgb_img(depth) for depth in all_depths]

    # # 拼接图像（列数可以自定义，比如 ncols=5）
    # stitched_image = stitch_images(rgb_images, ncols=5)

    # # 保存结果
    # stitched_image.save("all_depths_visualized-original3dgs.png")
    # print("拼接图已保存为 all_depths_visualized.png")
