...
def safe_interpolate(output, scale_factor=2, mode="bilinear", align_corners=False):
    max_elements = 2**31 - 1  # PyTorch 限制
    B, C, H, W = output.shape
    target_H = int(H * scale_factor)
    target_W = int(W * scale_factor)
    total_elements = B * C * target_H * target_W

    if total_elements > max_elements:
        # 计算每块最多可以保留的通道数
        max_C = max_elements // (B * target_H * target_W)
        # 至少为1通道
        max_C = max(1, max_C)

        parts = torch.chunk(output, chunks=(C + max_C - 1) // max_C, dim=1)
        parts_interp = [
            F.interpolate(p, scale_factor=scale_factor, mode=mode, align_corners=align_corners)
            for p in parts
        ]
        output_interp = torch.cat(parts_interp, dim=1)
    else:
        output_interp = F.interpolate(output, scale_factor=scale_factor, mode=mode, align_corners=align_corners)
...
class FeatureFusionBlock_custom(nn.Module):
    ...
    def forward(self, *xs):
        ...
        if self.width_ratio != 1:
            ...
            output = safe_interpolate(output, scale_factor=2, mode="bilinear", align_corners=self.align_corners)
    ...
    def __init__(self,
        ...
        if self.head_type == 'regression':
            ...
            self.head = nn.Sequential(
                ...
                nn.ReLU(True),
                nn.Conv2d(last_dim, last_dim // 2, kernel_size=1, stride=1, padding=0)
            )
