...
def normalize_pointcloud_bs(pts, valid_mask, norm_mode='avg_dis'):
    ...
    nan_pts1, nnz1 = invalid_to_zeros(pts, valid_mask, ndim=3)
    all_pts = nan_pts1
    ...
    norm_factor = all_dis.sum(dim=1) / (nnz1 + 1e-8)

    norm_factor = norm_factor.clip(min=1e-8)
    while norm_factor.ndim < all_pts.ndim:
        norm_factor.unsqueeze_(-1).unsqueeze_(-1)
