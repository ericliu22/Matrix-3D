...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, kernel_size, scale_factor):
    ...
    mask_path = os.path.join(model_path, name, "ours_{}".format(iteration), f"mask_{scale_factor}")
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        rendering_ret = render(view, gaussians, pipeline, background, kernel_size=kernel_size)["render"]
        rendering = rendering_ret[:3, :, :]
        gt = view.original_image[0:3, :, :]
        torchvision.utils.save_image(rendering, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
        torchvision.utils.save_image(gt, os.path.join(gts_path, '{0:05d}'.format(idx) + ".png"))

        # save depth mask, white mask
        depth = rendering_ret[6, :, :]
        accumlated_alpha = rendering_ret[7, :, :]

        depth_map = depth #H, W]
        depth_min = torch.min(depth_map)
        depth_max = torch.max(depth_map)
        depth_normalize = (depth_map - depth_min) / (depth_max - depth_min) * 255
        depth_normalize = Image.fromarray(depth_normalize.cpu().numpy().astype(np.uint8), mode="L")
        depth_normalize.save(os.path.join(depth_path, '{0:05d}'.format(idx) + ".png"))
        # import pdb; pdb.set_trace()

        accumlated_alpha = rendering_ret[7, :, :, None]
        ...
        torchvision.utils.save_image(colored_accum_alpha, os.path.join(mask_path, '{0:05d}'.format(idx) + ".png"))
