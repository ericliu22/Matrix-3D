...
@torch.no_grad()
def get_frustum_mask(points: torch.Tensor, cameras: List[Camera], near: float = 0.001, far: float = 1e6):
...
class GaussianModel:
    ...
    def training_setup(self, training_args):
        ...
        print(f"\n\nspatial lr scale: {self.spatial_lr_scale} {training_args.position_lr_init} {training_args.feature_lr} {training_args.opacity_lr} {training_args.scaling_lr} {training_args.rotation_lr}\n\n")
        l = [
    ...
    @torch.no_grad()
    def get_tetra_points(self, views: List[Camera], near: float = 0.001, far: float = 1e6):
